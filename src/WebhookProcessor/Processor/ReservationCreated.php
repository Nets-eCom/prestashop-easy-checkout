...
final readonly class ReservationCreated implements WebhookProcessorInterface
{
    ...
    public function process(WebhookInterface $webhook): void
    {
        ...
        if (!$details instanceof NexiCheckoutPaymentDetails) {
            throw new WebhookProcessorException(sprintf('Payment details not found for paymentId: %s', $paymentId));
        }

        if ($details->getOrderId() === null) {
            throw new WebhookProcessorException(sprintf('Order not created yet for paymentId: %s', $paymentId));
        }
        ...
        if ($order->getCurrentState() !== (int) \Configuration::get(OrderStateDictionary::PAYMENT_NEW)) {
            $this->logProcessMessage($event, 'order was already reserved', $paymentId);
