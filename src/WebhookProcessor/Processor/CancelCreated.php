...
final class CancelCreated implements WebhookProcessorInterface
{
    ...
    public function process(WebhookInterface $webhook): void
    {
        ...
        if ($order->getCurrentState() === (int) \Configuration::get('PS_OS_CANCELED')) {
            $this->logProcessMessage($event, 'order already cancelled', $paymentId);

            return;
        }

        $order->setCurrentState((int) \Configuration::get(OrderStateDictionary::PAYMENT_CANCELED));
